defmodule Queries do
  ...
  def big_cities_with_perfect_weather() do
    ...
    |> where([c, w], w.temp_lo >= 24 and w.temp_hi <= 30 and c.population > 1_000_000)
    |> Repo.all()
  end
  ...
  def small_cities_or_cold_cities() do
    City
    |> join(:inner, [c], w in Weather, c.id == w.city_id)
    |> where([c, w], w.temp_hi < 24 or c.population < 5_000)
    |> Repo.all()
  end
  ...
  def medium_cities_or_hot_cities_or_cold_small_cities() do
    ...
    |> or_where([c, w], c.population < 5_000 and w.temp_hi < 24 )
    |> Repo.all()
  end
end
